...
print("train the naive MobileNet model task by task")
...
# Iterate over the tasks (Task 1, Task 2, Task 3)
for task_idx, task_dataset in enumerate(tasks):
    ...
    peak_python_memory_mb = 0  # Peak Python memory in MB
    peak_gpu_memory_mb = 0     # Peak GPU memory in MB
    ...
    for step in progress_bar:
        ...
        progress_bar.set_description(
            f"Task {task_idx + 1} | Step {step + 1}/{len(progress_bar)} | loss: {loss.item():.4f} | "
            f"avg_loss: {np.mean(losses_train):.4f} | peak_py_mem: {peak_python_memory_mb:.2f} MB | "
...
print(f"Sequential training completed in {train_n_time:.2f} seconds\n")
...
pred_path = f"{gettempdir()}/pred_naive.csv"
...
metrics = compute_metrics_csv(eval_gt_path, pred_path, [neg_multi_log_likelihood,rmse, time_displace])
